...
int main( int argc, char** argv )
{
	...
	// size of new picture
	CvPoint Mydest_sz;
	// 547-3.1117.hiroshi: att_face .pgm size: row(112)xcol(92)
	Mydest_sz.x = atoi(argv[2]);	
	Mydest_sz.y = atoi(argv[3]);

	// 547.5.1120.hiroshi: quality type
	std::vector<int> qualityType;
	if (strcmp(argv[4], "jpg"))
	{
		// quality type JPG to save image
		qualityType.push_back(CV_IMWRITE_JPEG_QUALITY);
		qualityType.push_back(90);
	}
	else if (strcmp(argv[4], "pgm"))
	{
		// quality type PGM to save image
		qualityType.push_back(CV_IMWRITE_PXM_BINARY);
		qualityType.push_back(1);
	}
	
	// equalize histo color ?
	bEqHisto = atoi(argv[6]);
	 	...
	 	if ((imageName[nLen-1]=='g')&&(imageName[nLen-2]=='p')&&(imageName[nLen-3]=='j'))
	 	{
			...
			if( !frame.empty() )
			{
				...
				else 
				{
					...
					{
						char newName[16];
						// 547.5.1120.hiroshi: let user assign the format, and the filename will be all digit
						sprintf(newName,"%d.%s",index, argv[4]);
